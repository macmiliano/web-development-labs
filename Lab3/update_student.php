...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["email"])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $sql = "SELECT student_id, email FROM Students WHERE email = ? AND student_id != ?";
    ...
    if (empty($_POST["phone_number"])) {
        ...
        // Check if phone number contains only digits and is 9 digits long
        if (!preg_match('/^[0-9]{9}$/', $phone_number)) {
            $errors[] = "Phone number must be 7 digits";
        }
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssi", $name, $email, $phone_number, $student_id);
...
<body>
    <div class="container">
        <?php if (!empty($errors)): ?>
            ...
            <a href="javascript:history.back()" class="btn">Back to Edit Form</a>
        <?php endif; ?>
    </div>
</body>
</html>